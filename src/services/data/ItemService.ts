    ...
    try {
      ...
      this.realtimeListener = onSnapshot(
        ...
        (snapshot) => {
          ...
          // Only log changes in dev mode and limit to first 3
          if (__DEV__ && snapshot.docChanges().length <= 3) {
            ...
            console.log(`  (${snapshot.docChanges().length} changes - details omitted for performance)`);
          }
          ...
          if (__DEV__) console.log(`📊 Notifying ${this.changeCallbacks.length} subscribers with updated data`);
