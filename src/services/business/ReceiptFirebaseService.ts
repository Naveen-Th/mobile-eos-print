...
class ReceiptFirebaseService {
   ...
   */
  public async getAllReceipts(pageSize?: number): Promise<FirebaseReceipt[]> {
    try {
      ...
      const q = query(receiptsRef, orderBy('createdAt', 'desc'), limit(limitValue));
  ...
  public async fetchMoreReceipts(pageSize?: number): Promise<{ receipts: FirebaseReceipt[]; hasMore: boolean }> {
    try {
      ...
      const q = this.lastVisible
        ? query(receiptsRef, orderBy('createdAt', 'desc'), limit(limitValue), startAfter(this.lastVisible))
        : query(receiptsRef, orderBy('createdAt', 'desc'), limit(limitValue));
