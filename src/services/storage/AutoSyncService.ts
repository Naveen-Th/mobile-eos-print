...
class AutoSyncService {
  ...
  private readonly THROTTLE_DELAY = 0; // No delay for faster sync (batching already provides throttling)
  private readonly INITIAL_SYNC_LIMIT = 50; // Limit initial sync to most recent items for faster startup
  ...
  public async syncOnLogin(userId: string, options: SyncOptions = {}): Promise<void> {
    ...
    if (this.lastSyncAttempt && (now - this.lastSyncAttempt) < 5000) {
      console.log('⏸️ Sync called too soon after last sync, skipping...');
      return;
    }
  ...
  private async syncCollection(
    ...
    while (hasMore) {
      try {
        ...
        // Process batch in bulk
        try {
          await this.syncDocumentsBulk(snapshot.docs, localTable);
          synced += snapshot.docs.length;
        } catch (error) {
  ...
  private async syncReceiptsCollection(
    ...
    while (hasMore) {
      try {
        ...
        // Process receipts in bulk
        try {
          await this.syncReceiptsBulk(snapshot.docs);
          synced += snapshot.docs.length;
        } catch (error) {
          console.error(`Failed to sync receipts batch:`, error);
          // Fallback to individual processing
          for (const docSnap of snapshot.docs) {
            try {
  ...
  /**
   * Sync multiple documents in bulk (much faster)
   */
  private async syncDocumentsBulk(
    docs: any[],
    localTable: string
  ): Promise<void> {
    if (!database || docs.length === 0) return;

    const now = Date.now();
    const placeholders = docs.map(() => '(?, ?, ?, ?, ?, ?, ?, ?, 1)').join(', ');
    ...
    if (localTable === 'items') {
      ...
      database.runSync(
        `INSERT OR REPLACE INTO ${localTable} (id, firebase_id, item_name, price, stocks, created_at, updated_at, synced_at, is_synced) VALUES ${placeholders}`,
        values
      );
      ...
      const custPlaceholders = docs.map(() => '(?, ?, ?, ?, ?, ?, ?, ?, ?, 1)').join(', ');
      database.runSync(
        `INSERT OR REPLACE INTO ${localTable} (id, firebase_id, name, phone, email, address, created_at, updated_at, synced_at, is_synced) VALUES ${custPlaceholders}`,
        values
      );
    }
  }
  ...
  /**
   * Sync receipts in bulk (much faster)
   */
  private async syncReceiptsBulk(docs: any[]): Promise<void> {
    if (!database || docs.length === 0) return;

    const now = Date.now();
    
    // Build bulk insert for receipts
    const receiptPlaceholders = docs.map(() => '(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 1)').join(', ');
    ...
    docs.forEach(docSnap => {
      ...
      receiptIds.set(docSnap.id, receiptId);
    ...
    database.runSync(
      `INSERT OR REPLACE INTO receipts (id, firebase_id, receipt_number, customer_name, customer_phone, customer_address, subtotal, tax, total, date, print_method, printed, status, notes, created_at, updated_at, synced_at, is_synced) VALUES ${receiptPlaceholders}`,
      receiptValues
    );
    
    // Build bulk insert for receipt items
    const allItemValues: any[] = [];
    docs.forEach(docSnap => {
      const data = docSnap.data();
      const receiptId = receiptIds.get(docSnap.id)!;
    ...
    if (allItemValues.length > 0) {
      const itemPlaceholders = Array(allItemValues.length / 9).fill('(?, ?, ?, ?, ?, ?, ?, ?, ?)').join(', ');
      database.runSync(
        `INSERT OR REPLACE INTO receipt_items (id, receipt_id, item_id, item_name, quantity, price, total, created_at, updated_at) VALUES ${itemPlaceholders}`,
        allItemValues
      );
    }
  }
